...
fixed_sensor_pos = [] #[top,bottom]

agent_shadow_rel_pos = []
...
def move_rob(adj_Mat,num_Nodes,num_Edges,a,b,c,fileName,test=0):
	...
	sols = sols[[int(0.8*len(sols)):]]
	for sol in sols:
		rob_pos = [sol[ROB1_POS_INDEX]%1,sol[ROB2_POS_INDEX]%1]
		x_rob.append([sol[ROB1_POS_INDEX]%1,sol[ROB2_POS_INDEX]%1])


	rob_dist = [min(abs(rob_pos[0]-rob_pos[1]),1-abs(rob_pos[0]-rob_pos[1])) for rob_pos in x_rob]
